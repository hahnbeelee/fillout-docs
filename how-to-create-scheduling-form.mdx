...
<Steps>
  <Step title="Create an event">
    Click the `Scheduling` tab in the top toolbar of Fillout's dashboard followed by the `+ Scheduling Form` button. Then, Choose an [event type](/event-types).

    <Frame>
      ![](/images/scheduling/image-23.png)
    </Frame>
  </Step>
  <Step title="Edit the event details">
    Then, add a meeting **Title** and **Event duration** or enter a custom meeting length in minutes. You can also set a default **Meeting location** for an in-person or phone meeting, or add a video call link to your meetings.
  ...
  <Step title="Add more fields (Optional)">
  ...
  <Step title="Publish and share">
    Name your Scheduling form. Then, click `Publish` to share the link of your scheduling form.
